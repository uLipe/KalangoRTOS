...
#define BOARD_TIMER_STM0_BASE	0xF0001000u
#endif

#ifndef BOARD_TIMER_SRC_STM0_SR0
#define BOARD_TIMER_SRC_STM0_SR0	0xF0038300u
#endif
...
static ulmk_notif_t g_irq_notif;
...
static void stm0_arm_compare(volatile uint32_t *stm0, uint32_t delta_ticks)
{
	uint32_t now;
	uint32_t cmp;
	...
	now = stm0[stm0_off(STM0_TIM0)];
	cmp = now + delta_ticks;
	stm0[stm0_off(STM0_CMP0)] = cmp;
	stm0[stm0_off(STM0_ICR)]   = 0x00000001u;
}
...
static void timer_server(void *arg)
{
	...
	stm0 = (volatile uint32_t *)ulmk_mem_map(
	...
	stm0[stm0_off(STM0_CMCON)] = 0x0000001Fu;
	...
	for (;;) {
		...
		stm0_arm_compare(stm0, us_to_ticks(msg.words[0]));

		bits = 0u;
		ret  = ulmk_notif_wait(g_irq_notif,
				       1u << BOARD_TIMER_NOTIF_BIT, &bits);
		if (ret == ULMK_OK) {
			stm0[stm0_off(STM0_ISCR)] = 0x00000001u;
