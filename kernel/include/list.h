 ...
 *
 * Doubly-linked intrusive list — derived from Zephyr RTOS list primitives.
 *
 * NOTE: This file is licensed under Apache-2.0, unlike the rest of the ulmk
 * kernel tree (MIT).  Do not merge license terms when copying code out.
 */
 ...
 *
 * Lists are initialised so head and tail point at the list sentinel itself.
 */
...
/* Type-parameter variant for contexts without a container pointer lvalue. */
#define SYS_DLIST_CONTAINER_OF(__dn, __type, __n) \
	((__dn) ? CONTAINER_OF(__dn, __type, __n) : NULL)
/*
...
#define SYS_DLIST_PEEK_HEAD_CONTAINER_OF(__dl, __type, __n) \
	SYS_DLIST_CONTAINER_OF(sys_dlist_peek_head(__dl), __type, __n)
...
/**
 * @brief put a node in a known unlinked (self-referential) state
 *
 * A node initialised this way can be safely passed to sys_dlist_remove (it is a
 * no-op) and reports as not linked, so detach paths become idempotent.
 */
...
/**
 * @brief tell whether a node is currently part of a list
 *
 * Only meaningful for nodes initialised with sys_dnode_init and detached with
 * a matching re-init.
 */

static inline int sys_dnode_is_linked(const sys_dnode_t *node)
{
